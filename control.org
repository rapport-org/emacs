...
(when (equal (car rapport-emacs-opt-pkgs-mgmt) 'elpaca)
  ...
  (elpaca '(elpaca :repo "https://github.com/progfolio/elpaca.git" :ref nil :files (:defaults "elpaca-test.el" (:exclude "extensions")) :build (:not elpaca--activate-package)))
  (elpaca-wait)
  ;; </elpaca-init>
  (progn))
#+end_src
...
case ${1} in
	...
	;;
    ,*)
	cd $(dirname $(realpath ${RAPPORT_EMACS__URI_CONTROL}))
...
(use-package ggtags
  ...
  :ensure-system-package
  (global) ;; require GNU Global for File Indexing
  :custom
...
(use-package lsp-treemacs
  ...
  ;; ,FIXME: 20240217_NGa straight-keyword-inhibits-elpaca-load ;;  :straight (lsp-treemacs :type git :host github :repo "emacs-lsp/lsp-treemacs" :files ("*.el"))
  :config
...
(use-package lsp-mode
  ...
  #+end_src
**** configure YAML support in LSP-Mode
:PROPERTIES:
:ID:       1C021DEB-F6EF-497A-80D2-A2FE278C71C2
:END:
  #+begin_src emacs-lisp
(use-package emacs
  :ensure nil
  ...
  (lsp-yaml-schema-store-local-db (expand-file-name "lsp-yaml-schemas.json"
						    (expand-file-name "lsp-mode" rapport-uri-vault-var-emacs)))
  :config
  (setq lsp-yaml-server-command `("nix-shell" "-p" "yaml-language-server" "--run" "yaml-language-server --stdio"))
  (progn))

  #+end_src
**** configure Terraform support in LSP-Mode
:PROPERTIES:
...
(use-package flycheck-pycheckers
  ...
  #+end_src
**** enable =uv= for Python                                 :feature_preview:
**** enable Poetry for Python using =poetry.el=       :deprecation_candidate:
:PROPERTIES:
...
(use-package poetry
  ...
  (progn))
  #+end_src
**** enable Pipenv for Python                         :deprecation_candidate:
:PROPERTIES:
...
(use-package pipenv
  ...
  (progn))
      #+end_src
**** enable Blacken for Python
:PROPERTIES:
:ID:       0F242A26-60F8-4093-B98A-334FA57DBE85
:END:
      #+begin_src emacs-lisp
      ;; blacken only work from Python v3
      (use-package blacken
        ;:hook (python-mode) ;; #,LOG: 20201116_NGa remove hook which creates error on entering python-mode and breaks org-export
        )
      #+end_src
**** enable Pyenv for Python                          :deprecation_candidate:
:PROPERTIES:
:ID:       CABADB52-0D45-41DE-9996-8310FA0E51A9
:END:
      #+begin_src emacs-lisp
...
(use-package ox-taskjuggler
  :demand t
  :after (:all org-contrib)
  :ensure (org-contrib :main "lisp/ox-taskjuggler.el")
...
;; ,FIXME: 20240217_NGa straight-keyword-inhibits-elpaca-load ;;      :straight (org-protocol-capture-html :type git :host github :repo "alphapapa/org-protocol-capture-html")
      :config
      (progn))
  #+end_src
...
(use-package corfu-popupinfo
  ...
  :ensure (corfu-popupinfo :type git :host github
                        ...
                        :branch "main")
;; ,FIXME: 20240217_NGa straight-keyword-inhibits-elpaca-load ;;  :straight (corfu-popupinfo :type git :host github                         :repo "minad/corfu"                        :local-repo "corfu-popupinfo"                        :files ("extensions/corfu-popupinfo.el")                        :branch "main")
...
;; ,FIXME: 20240217_NGa straight-keyword-inhibits-elpaca-load ;;  :straight (corfu-terminal :type git :repo "https://codeberg.org/akib/emacs-corfu-terminal")
  ...
  #+end_src
***** COMMENT corfu-doc: The corfu-doc package displays the candidate documentation in a popup next to the Corfu popup, similar to company-quickhelp.
  #+begin_src emacs-lisp
;; ,ref: https://github.com/galeo/corfu-doc
